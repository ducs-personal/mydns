...
void zgethostbyname(unsigned char*, int);
...
static char dns_servers[10][32];
res_t answers[20], auth[20], addit[20];          //the replies from the DNS server
...
void _show_answers(int ans_count, int auth_count, int add_count)
{
    int i = 0;
...
void zgethostbyname(unsigned char *host, int query_type)
{
    ...
    qinfo = (question_t*) &buf[sizeof(dns_header_t) + (strlen((const char*) qname) + 1)];          //fill it

    qinfo->qtype = htons(query_type);          //type of the query , A , MX , CNAME , NS etc
    qinfo->qclass = htons(1);          //its internet (lol)

    printf("\nSending Packet...");
    if (sendto(sock, (char*) buf, sizeof(dns_header_t) + (strlen((const char*) qname) + 1) + sizeof(question_t), 0, (struct sockaddr*) &dest, sizeof(dest))
            < 0)
    {
    ...
    len = sizeof(dest);
    printf("\nReceiving answer...");
    if (recvfrom(sock, (char*) buf, 65536, 0, (struct sockaddr*) &dest, (socklen_t*) &(len)) < 0)
    {
    ...
    reader = &buf[sizeof(dns_header_t) + (strlen((const char*) qname) + 1) + sizeof(question_t)];
    ...
    //read authorities
    for (i = 0; i < ntohs(dnshdr->auth_count); i++)
    {
        auth[i].name = read_name(reader, buf, &stop);
        reader += stop;

        auth[i].resource = (res_data_t*) (reader);
        reader += sizeof(res_data_t);
    ...
    //read additional
    for (i = 0; i < ntohs(dnshdr->add_count); i++)
    {
        addit[i].name = read_name(reader, buf, &stop);
        reader += stop;

        addit[i].resource = (res_data_t*) (reader);
        reader += sizeof(res_data_t);
    ...
    _show_answers(ntohs(dnshdr->ans_count), ntohs(dnshdr->auth_count), ntohs(dnshdr->add_count));
...
void domain_format(unsigned char* dns, unsigned char* host)
{
    int lock = 0, i;
    strcat((char*) host, ".");
